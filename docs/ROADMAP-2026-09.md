...
| 18 | **Project/episode boundary** — first-class `project` tag on entries + work events; `brief`/`query`/`handoff_brief` accept a project filter; open-project listing. The primitive any large build consumer needs. *Amended 08-24 (Weighted Memory Tree 2608.20631):* task nodes gain a `parent_of` edge + a `folded` flag on done branches so `handoff_brief` collapses completed subtasks; the activation scoring itself stays consumer-side | LeanMem, Filesystem-Memory, WMT (2608.20631) | #1, #2 — domain-agnostic (a project is not a domain) |
