...
const TOOLS = {
  ingest: {
    ...
    schema: S({ path: { type: 'string' }, type: { type: 'string' }, title: { type: 'string' }, scope: { type: 'string' }, curated: { type: 'boolean' }, authority: { type: 'string', description: 'origin trust: operator|stack|doc|web (default doc; operator requires curated:true; never raised downstream)' }, ...PROJECT_W }, ['path']),
  ...
  query: {
    ...
    schema: S({ query: { type: 'string' }, tiers: { type: 'array', items: { type: 'string' } }, scopes: { type: 'array', items: { type: 'string' } }, ...PROJECT_R, functions: { type: 'array', items: { type: 'string' }, description: 'memory-function filter: working|episodic|semantic|procedural|prospective' }, limit: { type: 'number' }, maxTokens: { type: 'number' }, includeGraphContext: { type: 'boolean' }, minAuthority: { type: 'string', description: 'action-risk gate: exclude results below this origin authority (operator|stack|doc|web)' }, deep: { type: 'boolean', description: 'force the full hybrid pipeline (skip the progressive lexical-first sufficiency gate)' } }, ['query']),
    run: (a) => o.query(a.query, { tiers: a.tiers, scopes: a.scopes, functions: a.functions, limit: a.limit ?? 20, maxTokens: a.maxTokens, includeGraphContext: !!a.includeGraphContext, minAuthority: a.minAuthority, deep: !!a.deep, ...rproj(a) }),
  },
  feedback: {
  ...
  handoff_brief: {
    ...
    schema: S({ task: { type: 'string' }, profile: { type: 'string' }, scopes: { type: 'array', items: { type: 'string' } }, tiers: { type: 'array', items: { type: 'string' } }, ...PROJECT_R }, ['task']),
    run: (a) => o.handoffBrief({ task: a.task, profile: a.profile || 'local', scopes: a.scopes, tiers: a.tiers, ...rproj(a) }),
  },
  remember: {
  ...
  proactive_recall: { description: 'Trigger-less pre-turn recall: run the budgeted hybrid search on a raw user message and return {inject} ONLY if results clear the relevance threshold (else inject:null). Call this from a pre-turn hook to surface stored knowledge without the model spending a tool-call cycle. Records retrieval only for surfaced items.', schema: S({ message: { type: 'string' }, minScore: { type: 'number' }, maxTokens: { type: 'number' }, scopes: { type: 'array', items: { type: 'string' } }, ...PROJECT_R }, ['message']), run: (a) => o.proactiveRecall(a.message, { minScore: a.minScore, maxTokens: a.maxTokens, scopes: a.scopes, ...rproj(a) }) },
